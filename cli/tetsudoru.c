...
#define wprintf mywprintf

void myputwc(wchar_t wch, FILE *fp)
{
  char str[5];
  int n;
...
int start_game(int day)
{
  ...
  while(1){
    ...
    if (!isStation(ans[nans])){
      wprintf(L"\e[F %d : リストにありません\e[K\n", nans + 1);
      sleep(1);
      wprintf(L"\e[F");
      continue;
    }else{
      compare(problem, ans[nans], a);
      wprintf(L"\e[F %d : ", nans + 1);
      for (i = 0; i < 5; i++){
	wprintf(L"%s", color[a[i]]);
	putwc(ans[nans][i], stdout);
	wprintf(L"%s", color[0]);
      }
